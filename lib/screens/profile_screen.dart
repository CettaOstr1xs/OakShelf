...
class ProfileScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('My Reader Profile'),
        centerTitle: true,
      ),
      body: StreamBuilder<List<Book>>(
        ...
        builder: (context, bookSnapshot) {
          ...
          final avgRating = ratedCount > 0 ? (totalRating / ratedCount).toStringAsFixed(1) : '—';
              ...
              return SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // Grid Stats Card
                    _buildStatsGrid(context, readBooks.length, totalPages, avgRating, quotesCount),
                    const SizedBox(height: 24),

                    // Yearly Challenge Widget
                    _buildChallengeCard(context, readBooks.length),
                    const SizedBox(height: 24),
        ...
        child: Row(
          children: [
            ...
            Container(
              ...
              child: Icon(Icons.menu_book_rounded, color: theme.colorScheme.primary, size: 36),
            ),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 4),
                  Text(
                    'Firebase Account Sync Active',
                    style: theme.textTheme.bodySmall?.copyWith(fontSize: 10, color: Colors.green),
                  ),
                ],
              ),
            ),
  ...
  Widget _buildStatsGrid(BuildContext context, int completed, int pages, String avgRating, int quotes) {
    final theme = Theme.of(context);
    return GridView.count(
      ...
      children: [
        _buildStatCard(context, 'Completed', completed.toString(), Icons.check_circle_outline, Colors.green),
        _buildStatCard(context, 'Pages Read', pages.toString(), Icons.pages_outlined, theme.colorScheme.primary),
        _buildStatCard(context, 'My Avg Rating', avgRating, Icons.star_border_rounded, Colors.amber[700]!),
        _buildStatCard(context, 'Saved Quotes', quotes.toString(), Icons.format_quote_outlined, Colors.teal),
      ],
    );
  }
  ...
  Widget _buildChallengeCard(BuildContext context, int completed) {
    final theme = Theme.of(context);
    const int goal = 12; // Yearly reading challenge target
    ...
    return Card(
      ...
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(20),
        side: BorderSide(color: theme.colorScheme.primary.withOpacity(0.12), width: 1),
      ),
      child: Padding(
        padding: const EdgeInsets.all(20.0),
        child: Column(
