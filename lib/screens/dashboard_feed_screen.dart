...
class _DashboardFeedScreenState extends State<DashboardFeedScreen> with AutomaticKeepAliveClientMixin {
  @override
  ...
  final List<Map<String, String>> _quoteCollection = [
    ...
    {
      'text': 'Until I feared I would lose it, I never loved reading. One does not love breathing.',
      'author': 'Harper Lee'
    },
    {
  ...
  Future<void> _fetchRecommendationsOnce(List<Book> eligibleBooks) async {
    ...
    final recs = await widget.apiService.fetchBookRecommendations(eligibleBooks);
    ...
    return Scaffold(
      ...
      body: StreamBuilder<List<Book>>(
        ...
        builder: (context, bookSnapshot) {
          ...
          final readingBooks = books.where((b) => b.shelf == ShelfStatus.reading).toList();
          final readBooks = books.where((b) => b.shelf == ShelfStatus.read).toList();
          final wantBooks = books.where((b) => b.shelf == ShelfStatus.wantToRead).toList();

          final eligibleBooks = [...readBooks, ...wantBooks];
          final hasTenEligible = eligibleBooks.length >= 10;

          if (hasTenEligible && !_hasAttemptedRecommendations) {
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                _buildStatsRow(books.length, readingBooks.length, readBooks.length, wantBooks.length),
                ...
                _buildRecommendationsSection(hasTenEligible, eligibleBooks.length),
              ],
            ),
          );
        },
  ...
  Widget _buildQuoteInspirationCard(Map<String, String> quote) {
    ...
    return Card(
      ...
      shape: RoundedRectangleBorder(
        ...
        side: BorderSide(color: theme.colorScheme.primary.withOpacity(0.12), width: 1.5),
      ),
      child: Padding(
          ...
          children: [
            Row(
              ...
              children: [
                Row(
                  children: [
                    Icon(Icons.format_quote_rounded, color: theme.colorScheme.primary.withOpacity(0.3), size: 32),
                  ...
                  children: [
                    IconButton(
                      icon: _isSavingQuote
                          ...
                          : Icon(
                              _isQuoteSaved ? Icons.bookmark_rounded : Icons.bookmark_add_outlined,
                              size: 22,
                              color: _isQuoteSaved ? theme.colorScheme.primary : theme.colorScheme.primary.withOpacity(0.7),
                            ),
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                _buildStatColumn('Logged', total.toString()),
                _buildStatDivider(),
                _buildStatColumn('Reading', reading.toString()),
                _buildStatDivider(),
                _buildStatColumn('Finished', read.toString()),
              ],
            ),
          ],
        ),
      ...
      children: [
        ...
        if (readingBooks.isEmpty)
          Container(
            ...
            child: Column(
              children: [
                Icon(Icons.menu_book_rounded, size: 40, color: Colors.grey[300]),
                ...
                const SizedBox(height: 4),
                Text(
                  'Tap the plus button below to find and start a book!',
                            ...
                            child: Stack(
                              children: [
                                book.thumbnailUrl.isNotEmpty
                                    ? Image.network(book.thumbnailUrl, fit: BoxFit.cover, width: 60, height: 90)
  ...
  Widget _buildRecommendationsSection(bool hasTenEligible, int eligibleCount) {
    final theme = Theme.of(context);
      ...
      children: [
        ...
        if (!hasTenEligible) ...[
          Container(
            ...
            child: Column(
              children: [
                Icon(Icons.lock_clock_rounded, size: 36, color: theme.colorScheme.primary.withOpacity(0.5)),
                const SizedBox(height: 10),
                Text(
                ...
                Text(
                  'Add at least 10 books to your Wishlist or Finished shelf to unlock top-rated book recommendations matching your taste.',
                  style: theme.textTheme.bodySmall,
                  textAlign: TextAlign.center,
                ),
                const SizedBox(height: 14),
                ...
                ClipRRect(
                  ...
                  child: LinearProgressIndicator(
                    value: (eligibleCount / 10.0).clamp(0.0, 1.0),
                    backgroundColor: theme.colorScheme.primary.withOpacity(0.12),
                    valueColor: AlwaysStoppedAnimation<Color>(theme.colorScheme.primary),
                    minHeight: 8,
                  ),
